...
class CargoTaskManager {
    constructor(context, configurationManager, currentWorkingDirectoryManager, logger, stopCommandName) {
        this.configurationManager = configurationManager;
        this.currentWorkingDirectoryManager = currentWorkingDirectoryManager;
        this.logger = logger;
        this.outputChannelTaskManager = new output_channel_task_manager_1.OutputChannelTaskManager(configurationManager, logger.createChildLogger('OutputChannelTaskManager: '), stopCommandName);
        this.terminalTaskManager = new terminal_task_manager_1.TerminalTaskManager(context, configurationManager);
    }
    ...
    runCargo(command, args, force, reason) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.configurationManager.shouldExecuteCargoCommandInTerminal()) {
                ...
                const shouldShowOutputChannel = this.configurationManager.shouldShowRunningCargoTaskOutputChannel() &&
...
class CargoManager {
    constructor(context, configurationManager, currentWorkingDirectoryManager, logger) {
        ...
        this.cargoManager = new CargoTaskManager(context, configurationManager, currentWorkingDirectoryManager, logger.createChildLogger('CargoTaskManager: '), stopCommandName);
        this.customConfigurationChooser = new custom_configuration_chooser_1.default(configurationManager);
        this.logger = logger;
